...
export const DialogScrollableContent = ({ className, children, ...props }: React.HTMLAttributes<HTMLDivElement>) => {
  // Template
  return (
    <div className={cn('-mx-2 overflow-y-auto overflow-x-hidden px-2', className)} {...props}>
      {children}
    </div>
  )
}
  ...
  return (
    <DialogPortal data-slot='dialog-portal'>
      ...
      <DialogPrimitive.Content
        ...
        className={cn(
          'data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed top-[50%] left-[50%] z-50 grid max-h-[calc(100dvh_-_2rem)] w-full max-w-[calc(100%-2rem)] translate-x-[-50%] translate-y-[-50%] gap-4 rounded-lg border bg-background p-6 shadow-lg duration-200 data-[state=closed]:animate-out data-[state=open]:animate-in',
          className
        )}
        {...props}
      >
